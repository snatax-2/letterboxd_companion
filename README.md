...
- **`tests/e2e/design-audit-fixes.spec.js`** — correctifs issus de l'audit design : Export/Import déplacés dans Profil et fonctionnels, badge "Mode détaillé" en double retiré sans casser le changement de mode, sous-titre "Contexte de visionnage", chevron qui pivote sur "Personnaliser les pondérations", texte "Noter" intégré au cercle (plus de libellé externe), zéro violation d'accessibilité sérieuse sur les deux vues affectées.
- **`tests/e2e/ux-polish-round2.spec.js`** — bouton d'effacement fonctionnel sur les champs de recherche (formulaire de notation + historique, avec vérification que ça filtre bien la liste), zéro violation d'accessibilité sur le champ de recherche et la fenêtre modale avec verre dépoli.
- **`tests/e2e/full-ux-audit.spec.js`** — audit UX/design complet : zéro violation d'accessibilité sérieuse sur les 7 thèmes avec une vraie note en mode détaillé (révèle des éléments que les tests plus étroits ne couvraient pas), et vérifie qu'un champ `item.stars` manquant n'affiche jamais le mot "undefined" dans l'Historique.
- **`tests/e2e/tv-shows-phase1.spec.js`** — module Séries, Phase 1 (recherche + sélection de saison) : bascule Film/Série, recherche, choix de saison, bandeau récapitulatif, retour vers Film ; zéro violation d'accessibilité ; largeurs des 4 onglets de la barre de navigation strictement égales sur les 7 thèmes.
- **`tests/e2e/tv-shows-phase3.spec.js`** — module Séries, Phase 3 (notation de saison) : libellés et descriptions adaptés pour les 2 critères reformulés, sauvegarde et préremplissage au retour sur une saison, note globale de série calculée automatiquement (moyenne des saisons notées, exclut les non notées), zéro violation d'accessibilité sur les 7 thèmes.
- **`tests/e2e/tv-shows-phase4.spec.js`** — module Séries, Phase 4 (Historique scindé) : badge à deux compteurs (par série, pas par saison), bascule Films/Séries, carte de série avec note globale et saisons dépliables, réouverture d'une saison pour la re-noter (préremplissage vérifié), suppression avec confirmation, zéro violation d'accessibilité sur les 7 thèmes.
- **`tests/e2e/tv-shows-phase5.spec.js`** — module Séries, Phase 5 (statistiques) : bascule Films/Séries sur le tableau de bord, comptage et moyenne par série (pas par saison), radar avec libellés adaptés ("Final", "Cohérence"), Top Réalisateurs caché en mode Séries, distribution des notes, zéro violation d'accessibilité sur les 7 thèmes.
- **`tests/e2e/tv-shows-continue-widget.spec.js`** — widget "En cours" (onglet Noter, mode Série) : affiche le bon prochain épisode non vu, synopsis dépliable, validation qui passe à l'épisode suivant, détection automatique de la saison suivante une fois une saison terminée, série sans suite qui disparaît proprement, widget absent sans série en cours ou en mode Film, zéro violation d'accessibilité sur les 7 thèmes. Inclut aussi une vérification de non-régression sur l'ombre du bouton Noter dans la barre de navigation (ne doit plus déborder sur les onglets voisins).
- **`tests/e2e/tv-shows-history-parity.spec.js`** — parité Historique films/séries (1ère vague) : filtre par note branché sur les séries avec badge filtré correct, suppression d'une seule saison sans toucher aux autres, suppression de la dernière saison qui retire toute la série avec message adapté, zéro violation d'accessibilité sur les 7 thèmes.
...
- **`tests/e2e/ux-design-audit-fixes.spec.js`** — suite à un audit UX/design complet : cibles tactiles agrandies (étiquettes de contexte, filtres de tri, boutons pas-à-pas, bouton réglages) avec vérification de la zone tactile réelle par clic décalé, filtres de tri masqués quand il n'y a rien à trier (film et série), coquille "Twin Peaks" corrigée dans le placeholder du champ film, zéro violation d'accessibilité sur les 6 thèmes.
- **`tests/e2e/profile-empty-state-fixes.spec.js`** — points du Profil laissés de côté lors de l'audit précédent, traités ensuite : le radar ne réserve plus 160px vides à l'écran (se replie proprement, reprend sa taille dès que des notes détaillées existent), "Distribution des notes" affiche un message au lieu de 10 lignes à zéro, le bouton "Télécharger l'image" se désactive quand la carte est verrouillée et se réactive avec de vraies données, zéro violation d'accessibilité sur les 6 thèmes.
- **`tests/e2e/design-tokens-phase1.spec.js`** — Phase 1 du plan d'exécution de l'audit expert (fondations) : les affiches et photos se chargent correctement via la nouvelle fonction `tmdbImage()` centralisée (recherche, fiche film, fiche série, casting), une affiche manquante ne casse rien plutôt que d'afficher "undefined" dans l'URL.
- **`tests/e2e/phase2-ux-fixes.spec.js`** — Phase 2 du plan d'exécution de l'audit expert (UX) : l'espace vide de "Suggestions pour toi" (Découvrir) se réduit à une hauteur modeste au lieu de réserver 580px pour un simple message, restaure sa hauteur normale dès qu'une vraie suggestion existe ; la transition en fondu au changement d'onglet (Historique, Noter, Profil) laisse toujours le contenu final pleinement visible et fonctionnel, zéro violation d'accessibilité sur les 6 thèmes avant et après transition.
- **`tests/e2e/phase3-ui-fixes.spec.js`** — Phase 3 du plan d'exécution de l'audit expert (UI) : les 8 puces de thème (Découvrir) restent cliquables et ouvrent la bonne feuille de détail avec leurs nouvelles icônes SVG (plus d'emoji), les états vides compacts du Profil ("Top Réalisateurs", "Distribution des notes") migrés vers la fonction partagée affichent toujours le bon message, les états vides riches déjà fonctionnels (Historique, À voir) restent inchangés, zéro violation d'accessibilité sur les 6 thèmes.
- **`tests/e2e/phase4-desktop-layout.spec.js`** — Phase 4 du plan d'exécution de l'audit expert (responsive desktop) : l'Historique passe bien à 2 colonnes à partir de 1024px (confirmé par `getComputedStyle`), le glissement continue de fonctionner dans cette disposition, la barre de navigation atteint bien 1100px de large, et surtout — le mobile (390px) reste totalement inchangé (1 colonne, comportement identique à avant), zéro violation d'accessibilité sur les 6 thèmes en largeur desktop.
...
- **`tests/e2e/phase6-final-verification.spec.js`** — Phase 6 du plan d'exécution de l'audit expert (vérifications finales) : balayage complet des 5 écrans principaux sur les 6 thèmes, en mobile ET en desktop (12 combinaisons), zéro violation d'accessibilité sérieuse partout. A aussi débusqué un vrai bug préexistant (voir CHANGELOG) : une diapositive d'onboarding restait cliquable — invisible mais interceptant les clics — même quand la modale entière était fermée, à cause d'un `pointer-events: auto` appliqué dès l'initialisation plutôt que seulement à l'ouverture réelle.
...
- **`tests/e2e/tv-shows-search-opens-detail.spec.js`** — chercher une série dans Noter ouvre directement sa fiche détaillée (plus de puces de saison), pastille "Commencer la série" démarre à la Saison 1, capture du genre préservée, widget En cours alimenté, fiche mise à jour sur place.
