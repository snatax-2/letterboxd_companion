...
- **`tests/e2e/fdj-cache-retry.spec.js`** — un résultat d'anecdote en cache à `null` est retenté à chaque chargement (pas seulement le lendemain) ; un résultat déjà trouvé reste en cache sans nouvel appel ; le bouton "Vider le cache du Film du jour" dans Réglages force bien une nouvelle recherche.
