...
- **`tests/wiki-anecdote.test.js`** — logique d'extraction d'anecdotes Wikipédia (`api/_wikiAnecdote.js`) testée contre des fixtures réalistes (le sandbox n'a pas accès réseau à Wikipédia) : priorité Anecdotes > Production, accents ignorés, sections trop courtes rejetées, nettoyage des artefacts de notes, coupe propre en fin de phrase, candidats de désambiguïsation dans le bon ordre. Ajout : verrouille la priorité du titre français sur l'original pour la génération des candidats. Ajout : verrouille l'ordre de priorité des sections (Tournage/Genèse avant Production générique), vérifié contre la vraie structure d'un article Wikipédia réel.
- **`tests/e2e/wiki-anecdote.spec.js`** — une vraie anecdote Wikipédia s'affiche avec son attribution et prend le pas sur les faits TMDb ; repli propre si aucune n'est trouvée ; mise en cache journalière (pas de second appel).
- **`tests/e2e/fdj-cache-retry.spec.js`** — un résultat d'anecdote en cache à `null` est retenté à chaque chargement (pas seulement le lendemain) ; un résultat déjà trouvé reste en cache sans nouvel appel ; le bouton "Vider le cache du Film du jour" dans Réglages force bien une nouvelle recherche. Selecteurs mis a jour vers .fdj-anecdote-card apres l'habillage visuel.
- **`tests/e2e/desktop-layout.spec.js`** — verrouille le correctif d'affichage PC : le carrousel Tendances (10 films) ne fait plus deborder la page en largeur bureau (1440px), et aucun chevauchement entre les deux colonnes sur les 4 onglets.
