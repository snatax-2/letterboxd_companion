...
async function editorialCopy(films, geminiKey) {
  const fallback = { title: 'Regards du monde', intro: 'Quatre œuvres venues d’horizons différents, réunies ce mois-ci par leur exigence de cinéma.' };
  ...
  const prompt = `Tu es l’éditeur cinéphile de LUDEX. À partir de ces quatre films internationaux, trouve un lien thématique réellement défendable. Réponds uniquement par JSON : {"title":"2 à 7 mots","intro":"une phrase de 180 caractères maximum"}. N’invente aucun fait.\n\n${catalogue}`;
  try {
...
export default async function handler(req, res) {
  ...
  const chosenCountries = seededShuffle(COUNTRIES, seedForMonth(month)).slice(0, 4);
  ...
  if (films.length < 4) return res.status(502).json({ error: 'Sélection internationale indisponible.' });
